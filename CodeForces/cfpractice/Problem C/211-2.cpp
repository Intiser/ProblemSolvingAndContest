...
int main(){
   ...
   if(siz >1)
    t.pb(s[1]);
   if(siz>2 && (s[2]!=s[1] || s[2] != s[0]) ) t.pb(s[2]);
   for(int i=3,j = t.size();i<siz;i++){
         if(t[j-3] == t[j-2] && t[j-1] == s[i]) continue ;
         if(t[j-1] == t[j-2] && t[j-1] == s[i]) continue ;
         t.pb(s[i]);
         j++;
   }
   //for(int i=0;i<siz;i++) if(fl[i]==0) t.pb(s[i]);
   cout<<t<<endl;
}
