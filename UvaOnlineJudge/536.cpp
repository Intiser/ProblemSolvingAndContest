...
string pre; // root - left - right
string in;  // left - root - right

int g[2000][2];

bool side(char c,char root){
    int siz = in.size();
    FORL(i,0,siz){
        if(root == in[i]) return false;
        if(c == in[i] ) return true;
    }
}

void addd(int root,int c){
    //cout<<root<<" "<<g[root][0]<<" "<<g[root][1]<<endl;
    if(side(c,root) == true){
        if(g[root][0] == -1) {
            g[root][0] = c;
            return;
