    ...
    while(!q.empty()){
        ...
        FORE(j,1,k)
            FORL(i,0,4){
                ...
                int ny = p.y + (dy[i]*j);
                if(isValid(nx,ny)){
                    if( mat[nx][ny]>mat[p.x][p.y] && lvl[nx][ny] < lvl[p.x][p.y] + mat[nx][ny]){
                        lvl[nx][ny] = lvl[p.x][p.y] + mat[nx][ny];
                        np.x = nx;
                        np.y = ny;
                        q.push(np);
        ...
        if(fns < lvl[p.x][p.y]) fns = lvl[p.x][p.y];
    }
}
