...
void tester(){
    int siz = primes.size();
    for(int i=0;i<siz;i++) cout<<primes[i]<<" "<<powers[i]<<endl;
}
...
lli sumer(lli a,lli n){
    lli tot = a;
    tot =  ( (tot%mod) * ( bigmod(a,n,mod)  - 1 + mod )%mod )%mod;
    tot =  ( (tot%mod) * ( modInv(a-1,mod) )%mod )%mod;
    return tot;
}
...
lli ultimate(){
    ...
    int siz = powers.size();
    for(int i=0;i<siz;i++){
        lli tmp = ( (tot%mod) * ( sumer(primes[i],powers[i])%mod) )%mod;
        tot = ( (tot%mod) + (tmp%mod) )%mod;
    }
    return tot;
}
