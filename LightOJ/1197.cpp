...
lli funct(lli x,lli y){
    ...
    for(lli i=0;i<ind&&prime[i]<=y;i++){
        lli st = ceil((x * 1.0 )/ prime[i]) * prime[i];
        if(st == prime[i]) st = st + st;
        //cout<<st<<endl;
        for(lli j=st,k=st-x;j<=y;j=j+prime[i],k=k+prime[i]){
            ok[k]++;
            if(ok[k] == 1) cnt--;
        }
    }
