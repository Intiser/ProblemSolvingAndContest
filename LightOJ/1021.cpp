...
lli rec(int md,int val){
    //cout<<md<<" "<<val<<endl;
    if((val+1) == (1<<lst) ) return (md == 0);
    if(dp[md][val]!=-1) return dp[md][val];
    lli ret = 0;

    for(int i=0;i<lst;i++){
        if(!(val&(1<<i) )){
            int num = conv(c[i]) ;
            ret += rec( ( md*n + num)%k,val|(1<<i));
        }

    }
   // cout<<md<<" "<<val<<" "<<ret<<endl;
    return dp[md][val] = ret;
    //cout<<" ^^^ "<<dp[x][val][0]<<" "<<x<<" "<<val<<endl;
}
